...
async function createRoom() {
  ...
  const roomRef = await db.collection('rooms').doc('room1');
  ...
  document.querySelector(
    '#currentRoom').innerText = `Room joined successfully.`;
...
function joinRoom() {
  ...
  document.querySelector('#confirmJoinBtn').
    addEventListener('click', async () => {
      ...
      const db = firebase.firestore();
      const usersRef = db.collection('rooms').doc(roomId);

      usersRef.get()
        .then((docSnapshot) => {
          console.log("my room exisit", docSnapshot.exists);
          if (docSnapshot.exists) {
            joinRoomById(roomId);
          } else {
            createRoom();// create the document
          }
        });
...
async function hangUp(e) {
  ...
  window.open(audioRecorder.host + audioRecorder.redirectUrl, '_self')
